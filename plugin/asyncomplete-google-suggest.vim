...
function! s:handle_exit(ctx, opt, base, startcol, results, job_id, data, event_type) abort
  ...
  for l:m in l:result[1]
    call add(l:items, substitute(l:m[0], ' ', '', 'g'))
  endfor
  let l:line = getline('.')
...
function! s:handle_timer(opt, ctx, timer) abort
  ...
  let l:base = substitute(getline('.')[l:startcol : col('.')], '\.', ' ', 'g')
  echomsg localtime() l:base
...
let g:asyncomplete_preprocessor = add(get(g:, 'asyncomplete_preprocessor', []), function('s:my_asyncomplete_preprocessor'))
